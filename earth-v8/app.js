...
function videosOf(feature){const p=feature.properties||{};return arrayValue(p.videoIds).map(String).filter(id=>/^[A-Za-z0-9_-]{11}$/.test(id)).slice(0,3)}
...
function appendVideos(wrap,feature){
  const ids=videosOf(feature);
  if(!ids.length)return;
  const section=document.createElement('section');section.className='popup-videos';
  section.append(textNode('h4',ids.length===1?'Connected video':'Connected videos','video-heading'));
  ids.forEach((id,index)=>{
    const frameWrap=document.createElement('div');frameWrap.className='video-embed';
    const iframe=document.createElement('iframe');
    iframe.src=`https://www.youtube-nocookie.com/embed/${id}?rel=0`;
    iframe.title=`Siren video ${index+1}`;
    iframe.loading='lazy';
    iframe.referrerPolicy='strict-origin-when-cross-origin';
    iframe.allow='accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share';
    iframe.allowFullscreen=true;
    frameWrap.append(iframe);
    const fallback=textNode('a','Watch on YouTube','video-link');
    fallback.href=`https://www.youtube.com/watch?v=${id}`;
    fallback.target='_blank';
    fallback.rel='noreferrer';
    section.append(frameWrap,fallback);
  });
  wrap.append(section);
}
function detailPopup(feature,lngLat=feature.geometry.coordinates){
  ...
  new maplibregl.Popup({maxWidth:'520px'}).setLngLat(lngLat).setDOMContent(wrap).addTo(state.map);
}
async function clusterPopup(sourceId,feature,lngLat){
  ...
  wrap.append(textNode('p','Choose a siren below for its type, testing schedule, and connected videos. This group does not zoom automatically.','cluster-note'));
  ...
  leaves.forEach(leaf=>{
    ...
    const videoLabel=videosOf(leaf).length?' · video':'';
    button.append(textNode('strong',p.name||'Unnamed siren'),textNode('small',(p.sirenType||p.testingSchedule||sourcesOf(leaf).slice(0,2).join(' · ')||'Details unavailable')+videoLabel));
  ...
  popup=new maplibregl.Popup({maxWidth:'470px'}).setLngLat(lngLat).setDOMContent(wrap).addTo(state.map);
}
...
function startUs(){if(state.usStarted)return;state.usStarted=true;progress('International pins ready · streaming U.S. pins in background…');addSirenSource('usSirens','../data/pins-us.geojson','#f0b429');watchSource('usSirens',()=>{state.usReady=true;progress('All sirens ready.','ready');status('All sirens ready','ready');setTimeout(()=>$('#mapProgress').classList.add('hidden'),1800)})}
...
async function ensureSearchData(){
  ...
  state.searchData=[...(world.features||[]),...(us.features||[])];$('#startupNote').textContent='Search index ready. Videos remain lazy-loaded only when a siren popup is opened.';return state.searchData;
}
function renderResults(features){
  ...
  const fragment=document.createDocumentFragment();features.slice(0,200).forEach(feature=>{
    ...
    const videoLabel=videosOf(feature).length?' · video':'';
    button.append(textNode('strong',p.name||'Unnamed siren'),textNode('small',(p.sirenType||p.testingSchedule||sourcesOf(feature).slice(0,2).join(' · '))+videoLabel));
...
async function start(){
  try{
    ...
    await styleReady();
    progress('Loading international sirens…');
    addSirenSource('intlSirens','../data/pins-world.geojson','#28b9e7');
    watchSource('intlSirens',()=>{
      if(state.intlReady)return;state.intlReady=true;progress('International sirens ready.','ready');status('International sirens ready','ready');$('#startupNote').textContent='International pins are visible. Connected videos load only when you open a siren, so they do not slow the map.';startUs();
    });
